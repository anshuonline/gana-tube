...
@Component({
  ...
  template: `
    <div class="player-bar" [class.visible]="playerService.currentTrack() !== null" (click)="onPlayerBarClick($event)">
      ...
      <div class="top-progress-bar" (click)="onSeek($event)">
        <div class="progress-fill" [style.width.%]="progressPercent"></div>
        <div class="progress-thumb" [style.left.%]="progressPercent"></div>
      </div>

      <!-- Desktop Left Controls -->
      <div class="player-left-controls desktop-only">
        <button class="ctrl-btn" (click)="playerService.previous()" title="Previous"><svg lucideSkipBack [attr.size]="20"></svg></button>
        <button class="play-pause-btn" (click)="playerService.togglePlayPause()">
        ...
        <button class="ctrl-btn" (click)="playerService.next()" title="Next"><svg lucideSkipForward [attr.size]="20"></svg></button>
        <span class="time-label">{{ formatTime(playerService.currentTime()) }} / {{ formatTime(playerService.duration()) }}</span>
      </div>

      <!-- Center Info (Always visible, expands) -->
      <div class="player-center-info">
        <div class="album-art-wrapper" *ngIf="playerService.currentTrack() as track">
          <img class="album-art" [src]="track.thumbnailHigh || track.thumbnail" [alt]="track.title" referrerpolicy="no-referrer" />
          <div class="vinyl-overlay" [class.spinning]="playerService.playerState() === 'playing'"></div>
        </div>
        <div class="album-art-placeholder" *ngIf="playerService.currentTrack() === null"><svg lucideMusic2 [attr.size]="24"></svg></div>
      ...
      <div class="player-right-controls">
        ...
        <div class="desktop-actions desktop-only">
          <button class="ctrl-btn secondary" [class.active]="isCurrentTrackLiked()" (click)="toggleLike($event)" title="Like">
            <svg lucideHeart [attr.size]="18" [attr.fill]="isCurrentTrackLiked() ? 'currentColor' : 'none'"></svg>
          </button>
          <button class="ctrl-btn secondary" title="Dislike">
            <svg lucideThumbsDown [attr.size]="18"></svg>
          </button>
          <button class="ctrl-btn secondary" title="More options">
            <svg lucideMoreVertical [attr.size]="18"></svg>
          </button>
          
          <!-- Additional standard desktop controls -->
          <div class="desktop-extra-controls">
            ...
            <input type="range" class="volume-slider" min="0" max="100" [value]="playerService.isMuted() ? 0 : playerService.volume()" (input)="onVolumeChange($event)" title="Volume" />
            
            <button class="ctrl-btn secondary" [class.active]="playerService.repeatMode() !== 'none'" (click)="playerService.toggleRepeat()">
            ...
            <button class="ctrl-btn secondary" [class.active]="playerService.isShuffled()" (click)="playerService.toggleShuffle()"><svg lucideShuffle [attr.size]="18"></svg></button>
            <button class="ctrl-btn secondary" [class.active]="showQueue()" (click)="toggleQueue()" title="Play Queue"><svg lucideListMusic [attr.size]="18"></svg></button>
            <button class="ctrl-btn chevron-down" (click)="toggleFullScreen()" title="Expand"><svg lucideChevronDown [attr.size]="20"></svg></button>
          </div>
        </div>

        <!-- Mobile Actions -->
        <div class="mobile-actions mobile-only">
          <button class="play-pause-btn" (click)="playerService.togglePlayPause(); $event.stopPropagation()">
            <div class="spinner" *ngIf="playerService.playerState() === 'loading'"></div>
            <ng-container *ngIf="playerService.playerState() !== 'loading'">
              <svg *ngIf="playerService.playerState() === 'playing'" lucidePause [attr.size]="24"></svg>
              <svg *ngIf="playerService.playerState() !== 'playing'" lucidePlay [attr.size]="24"></svg>
          ...
          <button class="ctrl-btn" (click)="playerService.next(); $event.stopPropagation()" title="Next">
    ...
    <div class="fullscreen-overlay" [class.active]="isFullScreen()"
      ...
      <!-- Header Controls -->
      <div class="fs-header">
        <button class="fs-close-btn" (click)="toggleFullScreen()" title="Close Fullscreen">
          <svg lucideChevronDown [attr.size]="28"></svg>
        </button>
        <div class="fs-header-tabs">
          <button class="ctrl-btn secondary" title="More options">
      ...
      <div class="fs-container" [class.with-queue]="showFSQueue()">
        ...
        <div class="fs-queue-section" *ngIf="showFSQueue()">
          ...
          <div class="fs-queue-list">
            <div class="fs-queue-item" *ngFor="let track of playerService.queue(); let i = index" [class.active]="playerService.currentIndex() === i" (click)="playerService.playFromQueue(i)">
              ...
              <button class="fs-queue-remove-btn" (click)="onRemoveFromQueue($event, i)">
        ...
        <!-- Player View -->
        <div class="fs-player-section" *ngIf="!showFSQueue()">
          ...
          <div class="fs-info-section">
            ...
            <div class="fs-playback-controls">
              <button class="fs-ctrl-btn secondary" [class.active]="playerService.isShuffled()" (click)="playerService.toggleShuffle()">
                <svg lucideShuffle [attr.size]="24"></svg>
              </button>
              <button class="fs-ctrl-btn" (click)="playerService.previous()">
                <svg lucideSkipBack [attr.size]="32"></svg>
              </button>
              <button class="fs-play-btn-large" [class.loading]="playerService.playerState() === 'loading'" (click)="playerService.togglePlayPause()">
                <div class="spinner" style="width: 32px; height: 32px; border-width: 3px;" *ngIf="playerService.playerState() === 'loading'"></div>
                <ng-container *ngIf="playerService.playerState() !== 'loading'">
                  <svg *ngIf="playerService.playerState() === 'playing'" lucidePause [attr.size]="40"></svg>
                  <svg *ngIf="playerService.playerState() !== 'playing'" lucidePlay [attr.size]="40"></svg>
                </ng-container>
              </button>
              <button class="fs-ctrl-btn" (click)="playerService.next()">
                <svg lucideSkipForward [attr.size]="32"></svg>
              </button>
              <button class="fs-ctrl-btn secondary" [class.active]="playerService.repeatMode() !== 'none'" (click)="playerService.toggleRepeat()">
        ...
        <div class="fs-bottom-tabs">
          <button class="fs-tab-btn" [class.active]="showFSQueue()" (click)="toggleFSQueue()">Up next</button>
          <button class="fs-tab-btn" [class.active]="!showFSQueue()" (click)="toggleFSQueue()" *ngIf="showFSQueue()">Player</button>
          <button class="fs-tab-btn" *ngIf="!showFSQueue()">Lyrics</button>
          <button class="fs-tab-btn" *ngIf="!showFSQueue()">Related</button>
        </div>
      </div>
      <!-- Toast Notification -->
